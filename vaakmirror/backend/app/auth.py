...
def _check_active(db: Session, table: str, row_id: str) -> bool:
    ...
    row = db.execute(text(f"SELECT is_active FROM {table} WHERE id = :id"), {"id": row_id}).first()
...
def get_current_therapist_id(
    ...
    if not therapist_id or not _check_active(db, "therapists", therapist_id):
...
def get_current_patient_id(
    ...
    if not patient_id or not _check_active(db, "patients", patient_id):
...
def get_current_identity(
    ...
    if not _check_active(db, table, sub):
        raise HTTPException(status_code=401, detail=f"{kind.title()} not found or inactive")
    return kind, sub


def assert_therapist_owns_patient(db: Session, therapist_id: str, patient_id: str) -> None:
    row = db.execute(text("SELECT therapist_id FROM patients WHERE id = :id"), {"id": patient_id}).first()
    if not row:
...
def get_patient_summary(db: Session, patient_id: str) -> dict | None:
    row = db.execute(
